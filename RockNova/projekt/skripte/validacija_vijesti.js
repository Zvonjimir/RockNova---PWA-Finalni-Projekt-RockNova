document.addEventListener("DOMContentLoaded", function () {
  ...
  spremiBtn.onclick = (event) => {
    ...
    if (naziv.value.length < 5 || naziv.value.length > 60) {
      slanje_forme = false;
      naziv.style.border = "1px dashed red";
      document.getElementById("porukaNaziv").innerHTML =
        "Naslov vijesti mora imati između 5 i 60 znakova!<br>";
    } else {
      naziv.style.border = "1px solid green";
      document.getElementById("porukaNaziv").innerHTML = "";
    }

    if (kratkiOpis.value.length < 10 || kratkiOpis.value.length > 250) {
      slanje_forme = false;
      kratkiOpis.style.border = "1px dashed red";
      document.getElementById("porukaKratkiOpis").innerHTML =
        "Kratki opis mora imati između 10 i 250 znakova!<br>";
    } else {
